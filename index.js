...
client.on(Events.InteractionCreate, (interaction) => {
  ...
  if (interaction.customId === backId) {
    ...
    embed = generateEmbedPlayList(
      lastIndexOnPage,
      lastIndexOnPage + config.PLAYLIST_PAGE_SIZE
    );
  }
  ...
  interaction.update({
    embeds: [embed],
    components: [
      new ActionRowBuilder().setComponents([backButton, forwardButton]),
    ],
  });
});
...
function generateEmbedPlayList(start, end) {
  ...
  let playListSongs = getPlayListSongs(
    start ?? 0,
    end ?? config.PLAYLIST_PAGE_SIZE
  );
  let embedMessage = new EmbedBuilder().setTitle("Плейлист");
  playListSongs.forEach((song) => {
    embedMessage.addFields({
      name: "\u200b",
      value: song.title + song.status,
      inline: false,
    });
  });
  return embedMessage;
...
async function play(guild) {
  if (songs.length !== 0) {
    ...
    if (
      data.videoDetails.liveBroadcastDetails &&
      data.videoDetails.liveBroadcastDetails.isLiveNow
    ) {
      options = {
        highWaterMark: 1 << 25,
